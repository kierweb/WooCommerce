<?php
	 ...
	 * Gateway class
	 */
	class WC_p3 extends WC_Payment_Gateway {
		 ...
		 */
		public function generate_p3_form($order_id) {
			if ($this->type == 'hosted') {
				echo $this->generate_p3_hosted_form($order_id);
			} else if ($this->type == 'iframe') {
				echo $this->generate_p3_embedded_form($order_id);
			} else if ($this->type == 'direct') {
				echo $this->generate_p3_direct_form($order_id);
			} else {
				return null;
			}
		}
		 ...
		 * Hosted form
		 */
		public function generate_p3_hosted_form($order_id) {
			global $woocommerce;

			$order     = new WC_Order($order_id);
			$redirect  = add_query_arg('wc-api', 'WC_p3', home_url('/'));
			$callback  = add_query_arg('wc-api', 'WC_p3_Callback', home_url('/'));
		 ...
		 * Embedded form
		 */
		public function generate_p3_embedded_form($order_id) {
			global $woocommerce;
		 ...
		 * Direct form step 1
		 */
		public function generate_p3_direct_form($order_id, $errors = array()) {
		 ...
		 * Direct form step 2
		 */
		public function generate_p3_direct_form_step2($order_id, $request = array()) {
		...
		function receipt_page($order) {
			...
			if ($step === 2 && !$is3DS) {
				...
				if (count($errors) > 0) {
					...
					echo $this->generate_p3_direct_form($order, $errors);

				} else {

					echo $this->generate_p3_direct_form_step2($order, $_REQUEST);
				...
				echo $this->generate_p3_form($order);
